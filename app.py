...
SECRET_KEY = os.getenv("SECRET_KEY", "default-fallback-key-123")
...
# =========================
if not MONGO_URI:
    logging.error("CRITICAL: MONGO_URI is not set in Environment Variables!")
    # Если на Render забыли прописать переменную, сервер будет пытаться 
    # упасть красиво, но без базы он не жилец.
    client = None
else:
    try:
        # Устанавливаем таймаут 5 секунд, чтобы не ждать вечно
        client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000)
        client.admin.command('ping') # Проверка реального коннекта
        logging.info("SUCCESS: Connected to MongoDB Atlas!")
    except Exception as e:
        logging.error(f"DATABASE ERROR: {e}")
        client = None

db = client["kadrgram_database"] if client else None
users_table = db["users"] if db is not None else None
messages_table = db["messages"] if db is not None else None
